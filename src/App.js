...
const KEY = "ed210c00";
//x-x-x-x-x-x-  App component   x-x-x-x-x-x-x-
export default function App() {
  ...
  // }); // here this func is initialized on only initial render and not on subsequent re-render.
  // and always pass a func instead of calling the func inside useState()
  ...
  //storing movie data in local storage
  //localStorage.setItem("watched", JSON.stringify([...watched,movie]));  need to store data into string as localstorage reads data in key pairs which are usually string....

  function handleDeleteWatched(id) {
    setWatched((watched) => watched.filter((movie) => movie.imdbID !== id));
  }

  //setting local storage using useEffect()
  // useEffect(
  //   function () {
  ...
  // fetch promises using fetch() & .then
  // useEffect(function () {
  ...
  // fetch promises using async function
...
//x-x-x-x-x-x-x- Main Component is here x-x-x-x-x-x-x-x
...
function Movie({ movie, onSelectMovie }) {
  return (
    //here we need to pass a function not just call it....
